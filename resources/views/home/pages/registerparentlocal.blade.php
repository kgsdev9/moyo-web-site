        ...
        <div class="row pb-1 pb-sm-3 pb-lg-4">
            <div class="col-lg-4 pe-xxl-4">
                ...
                <p class="fs-lg pb-4 mb-0 mb-sm-2">
                    Merci de remplir ce formulaire pour vous inscrire en tant que parent ivoirien vivant en Côte d’Ivoire.
                </p>
            </div>
            <div class="col-lg-8 col-xl-7 offset-xl-1">
                <form class="row g-4 needs-validation" novalidate>
                    ...
                    <div class="col-sm-6">
                        ...
                        <input class="form-control form-control-lg" type="text" placeholder="Votre profession"
                            id="profession" x-model="form.profession" />
                        <span class="text-danger" x-show="errors.profession" x-text="errors.profession"></span>
                    </div>
                    ...
                    <div class="col-sm-12">
                        ...
                        <textarea class="form-control form-control-lg" rows="4" placeholder="Ville, commune, quartier, détails..."
                            id="adresseLongue" x-model="form.adresse"></textarea>
...
function RegisterParentLocal() {
    return {
        ...
        formatPhone() {
            ...
            // On limite à 10 chiffres
            numbers = numbers.substring(0, 10);
            // On ajoute les espaces tous les 2 chiffres
        ...
        validateForm() {
            ...
            if (!this.form.nomComplet.trim()) {
                this.errors.nomComplet = "Le nom complet est obligatoire.";
            }
            // Vérification du numéro : doit avoir exactement 10 chiffres
            let digits = this.form.telephone.replace(/\D/g, '');
            if (digits.length !== 10) {
                this.errors.telephone = "Le numéro doit contenir exactement 10 chiffres.";
            }
            if (!this.form.profession.trim()) {
                this.errors.profession = "La profession est obligatoire.";
            }
        ...
        async submitForm() {
            ...
            this.loading = true;
            try {
                let response = await fetch("{{ route('parents.local.store') }}", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                    },
                    // On envoie le numéro sans espaces au serveur
                    body: JSON.stringify({
                        ...this.form,
                        telephone: this.form.telephone.replace(/\D/g, '')
                    })
                });
                ...
                if (response.ok) {
                    this.form = { nomComplet: '', telephone: '', profession: '', adresse: '' };
                    this.step = 2;
                } else {
